...
{ 
	var firstTime = true
			var t0: Long = 0
			val bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("twitter_info.txt"), "UTF-8"))
			...
			def write2Log(a: Array[(String, Long, Long, String, Int, Int)])
	{
		if (firstTime)
		{
	...
	def main(args: Array[String]) {
		...
		val apiKey = "06VbUtXmxCIycXgEJy6F2ZqeJ"
		val apiSecret = "R1OHvmRcKELyHBDLnGca1QOW1tAR2SgtGkkqhOi2KySeCexFOa"
		val accessToken = "925004856-ewsJKUawywszTXpHfA76XvYgOJWgIeR8jBBY77Aa"
		val accessTokenSecret = "pufuk7ZoQHSE8WIzDdIRdwzB3Hdp6KlTZwlHgXVkr34Xd"

		Helper.configureTwitterCredentials(apiKey, apiSecret, accessToken, accessTokenSecret)

		val conf = new SparkConf().setMaster("local[2]").setAppName("First Example")
		val ssc = new StreamingContext(conf, Seconds(5))
		val streams = TwitterUtils.createStream(ssc, None)
		val englishTweets = streams.filter(_.getLang() == "en")

		val reTweets = englishTweets.filter(_.isRetweet).window(Seconds(12 * 5), Seconds(5))
